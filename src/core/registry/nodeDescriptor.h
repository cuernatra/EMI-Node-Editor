...
struct NodeDescriptor
{
    ...
    CompileCallback              compile;        ///< Build this node normally.
    CompileFlowCallback          compileFlow;    ///< Build flow behavior (if the node controls execution).
    CompileOutputCallback        compileOutput;  ///< Build one specific output value.
    CompilePreludeCallback       compilePrelude; ///< Add declarations before the main run path.
    CompileTopLevelCallback      compileTopLevel; ///< Add a top-level definition (if any).
    DeserializeCallback          deserialize;    ///< Custom load when pins/fields are unusual.

    bool                         bypassFlowReachableCheck = false; ///< If true, allow reading value pins even when this node hasn't run yet.
